...
function isTextNode(node) {
    if (node && node.name !== 'script' && node.nodeType == 3 && nonWhitespaceMatcher.test(node.nodeValue)) {
        ...
        // if we have only one word and the first char is a number we skip it
        if (firstChar >= '0' && firstChar <= '9' && lastChar >= '0' && lastChar <= '9' && words == 1)
            return false;
...
LanguageBuilder.prototype.build = function(level, htmlFile) {
    ...
    nodeSet.forEach(function(node){
        ...
        if (innerHtml) {
            var newHtml = innerHtml.replace(text, "{% t " + key + " %}");

            targetNode.html(newHtml);
        }
    });
...
LanguageBuilder.prototype.export = function() {
