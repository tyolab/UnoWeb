...
NextJs.prototype.extract_partial = function ($, section, section_obj, options) {
    ...
    type = type || "section";
    let contentStr = "";
    let to_replace_partial = options.replace_partial || false;
    /** 
     * Get Heading up to 6
     */
    for (var i = 0; i < 7; i++) {
...
NextJs.prototype.build = function ($, fileName) {
    ...
    let common_dir = nextjs.components + path.sep + "common";
    if (!fs.existsSync(common_dir))
        fs.mkdirSync(common_dir, { recursive: true });
    fs.writeFileSync(common_dir + path.sep + "head.js", headStr);
