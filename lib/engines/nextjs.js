...
NextJs.prototype.build = function ($, fileName) {
    ...
    fs.writeFileSync(nextjs.components + path.sep + "common" + path.sep + "head.js", headStr);
     ...
     // write page
     var pageStr = `
import { getContentAndSettings, getSiteMenu, getSiteSettings, getFooter } from '../lib/content';
...
export default function ({ content, settings, children, className, ...rest }) {
    ...
    return (
        ...
        <Container className={cx(settings.styles.container)}>
            ...
            fs.writeFileSync(componentPath + path.sep + sectionVariable, bodyStr);
         }
         // });
     ...
     sectionNames.map((section, index) => {
        layoutStr += `
            <${section[0]} id="${sectionId}" content={${section[1]}.content} settings={${section[1]}.settings} />\n`;
        });
...
export default function ${name.charAt(0).toUpperCase()}${name.substr(1)}({
    ...
    return (
        <Layout menu={menu} settings={settings} footer={footer} >
            ${layoutStr}
        </Layout>
    );
}
